...
const std::string remapping_launch = "navigation_remapping.launch.py";
...
void startLaunch(const std::string &package, const std::string &launch_file) 
{
        ...
        if (launch_pid == 0) {
            // In child process
            #ifdef IS_PRODUCTION
                // Production (ရိုဘော့အစစ်) အတွက်: ပုံမှန်အတိုင်းပဲ Launch ခေါ်မည်
                execlp("ros2", "ros2", "launch", package.c_str(), launch_file.c_str(), (char *)NULL);
            #else
                // Simulation (Gazebo) အတွက်: use_sim_time:=True ပါ တစ်ပါတည်း ထည့်ပေးမည်
...
int main(int argc, char **argv)
{
    ...
    // Create a node
    auto node = rclcpp::Node::make_shared("which_nav_switcher", rom_robot_namespace);
    ...
    RCLCPP_INFO(rclcpp::get_logger("which_navs_server"), "debug_mode_: %s", debug_mode_ ? "true" : "false");
