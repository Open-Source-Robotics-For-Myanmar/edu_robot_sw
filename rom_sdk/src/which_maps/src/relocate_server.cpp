...
public:
  RelocateServer() : Node("relocate_server"), is_relocating_(false) {
    ...
    if (cfg_dir_env) {
      ...
      try {
        carto_config_dir_ = ament_index_cpp::get_package_share_directory(
                                robot_model_ + "_carto") +
                            "/config/";
      } catch (const std::exception &) {
        // Workspace for this package may not be sourced; use known install path
        carto_config_dir_ = "/home/buc_robot/software_ws/install/" +
                            robot_model_ + "_carto/share/" + robot_model_ +
                            "_carto/config/";
        RCLCPP_WARN(
            get_logger(),
    ...
    // MultiThreadedExecutor can therefore process service responses
    // (svc_cb_group_) while the subscriber group is held by the callback or
    // worker thread.
    sub_cb_group_ =
    ...
    RCLCPP_INFO(get_logger(),
                ...
                rel_topic.c_str(), carto_config_dir_.c_str(),
                carto_config_base_.c_str());
  }
...
private:
  ...
  void doRelocation(const PoseWCS::SharedPtr msg) {
    ...
    if (!callFinishTrajectory(active_id)) {
      RCLCPP_ERROR(get_logger(),
                   "[RelocateServer] finish_trajectory(%d) failed. Aborting.",
                   active_id);
      is_relocating_ = false;
      return;
    }
  ...
  bool getActiveTrajectoryId(int32_t &out_id) {
    ...
    auto future = states_client_->async_send_request(
        std::make_shared<GetStates::Request>());

    // The MultiThreadedExecutor (svc_cb_group_) resolves this future; we just
    // wait.
  ...
  bool callFinishTrajectory(int32_t trajectory_id) {
    ...
    if (future.wait_for(SVC_CALL_TIMEOUT) != std::future_status::ready) {
      RCLCPP_ERROR(get_logger(),
                   "[RelocateServer] finish_trajectory(%d) timed out.",
                   trajectory_id);
      return false;
    }
    ...
    RCLCPP_INFO(get_logger(), "[RelocateServer] Trajectory %d finished OK.",
                trajectory_id);
    return true;
  }

  // ── Helper: StartTrajectory with initial pose ────────────────────────────
  bool callStartTrajectory(const geometry_msgs::msg::Pose &pose,
    ...
    auto resp = future.get();
    if (resp->status.code != 0) {
      RCLCPP_ERROR(get_logger(), "[RelocateServer] start_trajectory error: %s",
                   resp->status.message.c_str());
      return false;
    }
