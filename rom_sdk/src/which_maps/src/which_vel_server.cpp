...
public:
    ...
    {
        publisher_ = this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel_stt_to_twist", 10);
        ...
        // Macro စစ်ပြီး Timer အမျိုးအစားကို ခွဲခြားသတ်မှတ်ခြင်း
            ...
            RCLCPP_INFO(this->get_logger(), "CmdVelServiceNode started in PRODUCTION mode (Using Wall Timer).");
        #else
            // Simulation အတွက်: Gazebo Clock (use_sim_time) ကို လိုက်နာမယ့် rclcpp::create_timer ကိုသုံးမယ်
            timer_ = rclcpp::create_timer(
                this, // shared pointer context ကို ရယူရန် (သို့မဟုတ် template သတ်မှတ်ချက်အရ "this" ဟု ရေးနိုင်သည်)
                this->get_clock(),
                std::chrono::milliseconds(50),
                std::bind(&CmdVelServiceNode::publish_cmd_vel, this));
            RCLCPP_INFO(this->get_logger(), "CmdVelServiceNode started in SIMULATION mode (Using Sim Timer).");
        #endif
        ...
        RCLCPP_INFO(this->get_logger(), "debug_mode_: %s", debug_mode_ ? "true" : "false");
