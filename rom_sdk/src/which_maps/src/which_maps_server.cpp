...
void startLaunch(const std::string &package, const std::string &launch_file) {
  if (debug_mode_) {
    RCLCPP_INFO(rclcpp::get_logger("which_maps_server"),
                "Starting launch file: %s/%s", package.c_str(),
                launch_file.c_str());
  }
...
// In child process
#ifdef IS_PRODUCTION
    execlp("ros2", "ros2", "launch", package.c_str(), launch_file.c_str(),
           (char *)NULL);
#else
    execlp("ros2", "ros2", "launch", package.c_str(), launch_file.c_str(),
           "use_sim_time:=true", (char *)NULL);
#endif
...
void shutdownLaunch() {
  if (launch_pid > 0) {
    if (debug_mode_) {
      RCLCPP_INFO(rclcpp::get_logger("which_maps_server"),
                  "Shutting down current launch process (PID: %d)...",
                  launch_pid);
    }
...
// INPUT ( which_map_do_you_have | save_map | select_map | mapping | navi |
// remapping )
...
void which_map_answer(
    const std::shared_ptr<rom_interfaces::srv::WhichMaps::Request> request,
  ...
  if (request->request_string == "which_maps_do_you_have") {
    ...
    // RCLCPP_INFO(rclcpp::get_logger("which_maps"), "%s",
    // package_directory.c_str());
    ...
    try {
      for (const auto &entry :
        ...
        if (entry.is_regular_file() && entry.path().extension() == ".yaml" &&
            entry.path().filename() !=
                "active_map.yaml") //&& entry.path().extension() == ".pgm" &&
      ...
      if (debug_mode_) {
        RCLCPP_INFO(rclcpp::get_logger("which_maps_server"),
                    "Sending : Response Status not OK");
      }
    }
  }
  ...
  /// ၂။ မြေပုံ save ပါ။
    ...
    if (debug_mode_) {
      RCLCPP_INFO(rclcpp::get_logger("which_maps_server"), "Saving map: %s",
                  map_name.c_str());
    }
    // Save the map
    // command စစ်ဆေးရန်။
    ...
    std::string cmd;
#ifdef IS_PRODUCTION
    cmd = "cd " + package_directory + " && rm -f " + map_name +
          "* && ros2 service call /" + rom_robot_namespace +
          "/write_state cartographer_ros_msgs/srv/WriteState \"{filename: '" +
          package_directory + map_name +
          ".pbstream', include_unfinished_submaps: true}\"";
#else
    cmd = "cd " + package_directory + " && rm -f " + map_name +
          "* && ros2 service call /" + rom_robot_namespace +
          "/write_state cartographer_ros_msgs/srv/WriteState \"{filename: '" +
          package_directory + map_name +
          ".pbstream', include_unfinished_submaps: true}\" --ros-args -p "
          "use_sim_time:=true";
#endif
    ...
    if (ret_code == 0) {
      if (debug_mode_) {
        RCLCPP_INFO(rclcpp::get_logger("which_map_server"),
                    "Map saver command 1 executed successfully.");
      }
      ...
      std::string cmd2 =
          ...
          package_directory + map_name +
          ".yaml && "
          "mv /home/buc_robot/software_ws/map.pgm " +
          package_directory + map_name +
          ".pgm 2>/dev/null || true && "
          "mv /home/buc_robot/software_ws/map.yaml " +
          package_directory + map_name + ".yaml 2>/dev/null || true";

      int ret_code2 = std::system(cmd2.c_str());
      if (ret_code2 == 0) {
        if (debug_mode_) {
          RCLCPP_INFO(rclcpp::get_logger("which_map_server"),
                      "Map saver command 2 executed successfully.");
        }
        // ၃။ ထွက်လာတဲ့ yaml ဖိုင်ထဲက image name အား တိုက်ရိုက် ဆောက်ထားတဲ့ pgm သို့
        // လွှဲပြောင်းပြင်ဆင်ခြင်း
        // (--copy သုံးထားခြင်းဖြင့် Device Busy သက်သာစေသည်)
        std::string cmd3 = "sed -i \"s|^image: .*\\.pgm|image: " + map_name +
                           ".pgm|\" " + package_directory + map_name + ".yaml";
        ...
        if (ret_code3 == 0) {
          if (debug_mode_) {
            RCLCPP_INFO(rclcpp::get_logger("which_maps_server"),
                        "sed command 3 Ok. all done.");
            response->status = 4;
          }

          // ၄။ အသစ်သိမ်းဆည်းလိုက်သော မြေပုံအား လက်ရှိသုံးမည့် Active Symlink အဖြစ်
          // ချက်ချင်းချိတ်ဆက်ခြင်း
          std::string ln_cmd = "ln -sf " + package_directory + map_name +
                               ".yaml " + package_directory +
                               "active_map.yaml && "
                               "ln -sf " +
                               package_directory + map_name + ".pgm " +
                               package_directory +
                               "active_map.pgm && "
                               "ln -sf " +
                               package_directory + map_name + ".pbstream " +
                               package_directory + "active_map.pbstream";
          std::system(ln_cmd.c_str());

          if (debug_mode_)
            RCLCPP_INFO(rclcpp::get_logger("which_maps_server"),
                        "Map saved and symlinked successfully.");
  ...
  /// ၃။ မြေပုံရွေးပါ။
    ...
    if (debug_mode_) {
      RCLCPP_INFO(rclcpp::get_logger("Open Map"),
                  "Sending : Response Status OK");
    }
    ...
    if (std::filesystem::exists(pgm_file) &&
        std::filesystem::exists(yaml_file) &&
        std::filesystem::exists(pbstream_file)) {
      if (debug_mode_) {
        RCLCPP_INFO(rclcpp::get_logger("Select Map"),
                    " : All required files exist for map:");
      }
      // [အဓိကအချက်] - sed နေရာတွင် Linux Symlink (ဖြတ်လမ်းဖိုင်) ဖြင့်
      // လမ်းကြောင်းပြောင်းလဲခြင်း active_map အမည်ဖြင့် သက်ဆိုင်ရာ မြေပုံဆီ လှည့်ပေးလိုက်သည်
      // (Container Mount Error ကင်းဝေးစေသည်)
      std::string symlink_cmd =
          "ln -sf " + yaml_file + " " + package_directory +
          "active_map.yaml && "
          "ln -sf " +
          pgm_file + " " + package_directory +
          "active_map.pgm && "
          "ln -sf " +
          pbstream_file + " " + package_directory + "active_map.pbstream";

      int link_ret = std::system(symlink_cmd.c_str());
      if (link_ret == 0) {
        if (debug_mode_) {
          RCLCPP_INFO(rclcpp::get_logger("Select Map"),
                      " : Map files Linking is ok:");
        }
...
#ifdef IS_PRODUCTION
        map_cmd = "ros2 service call /map_server/load_map "
                  "nav2_msgs/srv/LoadMap \"{map_url: '" +
                  path_to_active_yaml + "'}\" > /dev/null 2>&1";
#else
        map_cmd = "ros2 service call /map_server/load_map "
                  "nav2_msgs/srv/LoadMap \"{map_url: '" +
                  path_to_active_yaml +
      ...
      if (debug_mode_) {
        RCLCPP_INFO(rclcpp::get_logger("Select Map"),
                    " : Some required files are missing for map:");
      }
  ...
  /// ၄။ mapping mode change ပါ။
    ...
    if (current_mode != "mapping") {
      ...
      if (debug_mode_) {
        RCLCPP_INFO(rclcpp::get_logger("which_maps_server"),
                    "Mode '%s' is already active.", current_mode.c_str());
      }
    }
  }
  ...
  /// ၇။ သတ်မှတ်ချက်မရှိသော mode ဖြစ်ပါက
    ...
    if (debug_mode_) {
      RCLCPP_INFO(rclcpp::get_logger("which_maps_server"),
                  "Sending : Response Status not OK");
      RCLCPP_WARN(rclcpp::get_logger("which_maps_server"), "Unknown mode '%s'.",
                  request->request_string.c_str());
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  if (robot_name.empty()) {
    throw std::runtime_error("robot_name is empty. Please set ROM_ROBOT_MODEL "
  ...
  RCLCPP_INFO(rclcpp::get_logger("which_maps_server"), "debug_mode_: %s",
              debug_mode_ ? "true" : "false");

  auto node_options = rclcpp::NodeOptions();
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared(
      "which_maps_server", rom_robot_namespace, node_options);

  rclcpp::Service<rom_interfaces::srv::WhichMaps>::SharedPtr service =
      node->create_service<rom_interfaces::srv::WhichMaps>("which_maps",
                                                           &which_map_answer);

  // latch publisher
  global_publisher = node->create_publisher<std_msgs::msg::String>(
      "which_nav", rclcpp::QoS(1).transient_local());

  if (debug_mode_) {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Ready to answer maps.");
    RCLCPP_INFO(rclcpp::get_logger("which_maps_server"),
                "Fist Time trigger to Nav mode");
  }
