...
// const std::string bt_xml_file = ament_index_cpp::get_package_share_directory("rom_robot_autonomy") + "/trees/default.xml";
        ...
        if (config["waypoints"]) 
        {
            for (const auto& wp : config["waypoints"]) 
            {
                ...
                if (wp["frame_id"]) 
                {
                    ...
                    pose.header.frame_id = "map"; // Default frame if not specified
                }
                ...
                // Add the named goal pose to the blackboard
                blackboard->set(goal_pose_name, pose);
                //blackboard->set("my_goal_pose", goal_pose_);

                //waypoints_.push_back(pose); // Optionally store in a vector as well
...
int main(int argc, char **argv)
{
    ...
    auto node = rclcpp::Node::make_shared("autonomy_node"); // Create your ROS 2 node

    //std::vector<std::string> plugins = {"/path/to/your/plugin.so"};
    ...
    // Create an instance of the BehaviorTreeEngine
    ...
    // Load waypoints from YAML file and populate the blackboard
    load_yaml_file(waypoints_yaml_file, blackboard);

    auto tree = engine.createTreeFromFile(bt_xml_file, blackboard);

    // Enable Groot 1 monitoring
    BT::PublisherZMQ publisher(tree);//, 1666, 1667); // Ports for Groot visualization
    ...
    auto cancelRequested = []() -> bool {
        return false; // Change this if you want to allow canceling
    };


    // Run the behavior tree
    nav2_behavior_tree::BtStatus status = engine.run(&tree, onLoop, cancelRequested, std::chrono::milliseconds(100));
