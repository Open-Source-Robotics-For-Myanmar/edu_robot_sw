...
{
    bool incoming_ip = true;
    // Argument check for IP and port
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " <groot_ip> <zmq_port>" << std::endl;
        incoming_ip = false;
    }
    ...
    // Groot Publisher (ZMQ) ကို စတင်ခြင်း
    // (Optional) 100 milliseconds တိုင်း အပ်ဒိတ်ပို့ရန်အတွက် 100 ကို ထည့်သွင်းထားပါသည်။
    if(incoming_ip)
    {
        std::string groot_ip = argv[1];
        unsigned short zmq_port = static_cast<unsigned short>(std::stoi(argv[2]));

        BT::PublisherZMQ publisher(tree, zmq_port, 100, groot_ip);
    }
    else {
